...
![Crown rot on the cutting surfaces causing decay and premature ripening.](crown_rot.jpg)
![](12.jpg)
...
![Controlled atmosphere container.](CA_container.jpg){width=700px}
...
```{r graph1, echo=FALSE, fig.cap='Boxplot of crown rot incidence per month'}
...
```{r graph2, echo=FALSE, fig.cap="A plot of the mean temperatures per date for Piura, Peru"}
...
```
```{r graph3, echo=FALSE, fig.cap="Plot of accumulated degree.days per date for Piura, Peru."}
...
```{r add_plot, echo=FALSE, fig.cap="Accumulated degree days and crown rot."}
...
```
```{r, heatmap, echo=FALSE, fig.cap="A heatmap of crown rot against ADD and transit time(days)."}
#make geom_tile plot
p2 <- data_hm_PE %>%
...
data_PE %>% select(C, add, days, ca) %>% mutate(CA = as.numeric(ca)) %>% select(-ca) %>%
  drop_na() %>% corstars(.) %>% formattable(align = c("l", "r", "r", "r"), fixedWidth = 50)
```
...
```{r result_plot, echo=FALSE, fig.cap="Results of classification algorithms"}
...
```{r cfm, echo=FALSE, fig.cap="Heatmap of confusion matrix on perceptron model on final hold-out sample."}
...
p5
```
Figure \@ref(fig:cfm) visualises the confusion matrix. The model on a real sample achieves an accuracy of `r cfm_mlp_v$overall[1]` and a kappa value of `r cfm_mlp_v$overall[2]`. This is perhaps not impressive, but better than a random guess with an estimated accuracy of about 0.33 (3 classes). Besides, it is important that highs are as much as possible classified as highs, and not as lows, but if it is classified as a medium, it is less problematic. Of course, one could argue, that in that case it would be more logical to design it as a two-class problem. However, I have chosen for this approach to achieve a balance between reasonable accuracy of predicting mediums and highs, and not missing too many, as they can come with high costs, on the one hand, as well as not classifying lows incorrectly as medium or high and having to bear the additional cost of 1000 USD per container for operating under CA. If the economic relation between medium and high CR incidence was clearer, this could help inform such a decision. 
